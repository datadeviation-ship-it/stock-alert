...
- Alarm se šalje samo ako je:
    ...
    2) današnji open >= zadana razina
       (fallback: ako FMP jos nema open, koristi live price)
...
def provjeri(force=False):
    ...
    for item in DIONICE:
        ...
        if not quote:
            missing_open += 1
            print(f"{ticker:<8} prethodni close OK, ali nema quote podatka.")
            continue

        open_today = quote.get("open")
        live_price = quote.get("price")

        # FIX: FMP ne popunjava open odmah na otvaranju burze.
        # Ako open nije dostupan ili je 0, koristimo live price kao fallback
        # da alarm stigne odmah u 09:31 ET a ne sat-dva kasnije.
        if not open_today or open_today <= 0:
            open_today = live_price
            price_source = "live_price"
        else:
            price_source = "open"

        if open_today is None or open_today <= 0:
            missing_open += 1
            print(f"{ticker:<8} prethodni close OK, ali nema ni open ni price podatka.")
            continue
        ...
        print(
            ...
            f"razina={razina:.2f} "
            f"prev_close={prev_close:.2f} "
            f"open={open_today:.2f} ({price_source}) "
